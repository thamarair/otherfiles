...
class Assessment_model extends CI_Model {
        ...
        { 
			$innow=date("Y-m-d H:i:s");
			$query = $this->db->query('CALL checkuserisactive("'.$userid.'","'.$login_session_id.'","'.$innow.'")');
			mysqli_next_result($this->db->conn_id);
			//echo $this->db->last_query(); exit;
			return $query->result();
        }
		...
		public function getRandomGames($check_date_time,$game_plan_id,$game_grade,$school_id)
		 ...
		 {
			$query = $this->db->query("delete FROM rand_selection WHERE gp_id = '".$game_plan_id."' AND grade_id = '".$game_grade."' AND school_id = '".$school_id."' and user_id='".$userid."'");
		 }
		...
		//echo $this->db->last_query(); exit;
		 ...
		 { 
			$query = $this->db->query("SELECT j.id, g.gs_id as skill_id, j.name AS skill_name, g.gid, g.gname FROM users AS a
				...
				JOIN class_plan_game AS d ON d.class_id='".$game_grade."' AND d.plan_id = '".$game_plan_id."'
				
				JOIN games AS g ON d.game_id = g.gid 
				
				JOIN category_skills AS j ON g.gs_id = j.id   
				
				WHERE a.id = '".$uid."' AND g.gc_id = '".$catid."' and j.id = '".$skill_id."' 
				
				and g.gid not in (SELECT gid FROM rand_selection WHERE gp_id = '".$game_plan_id."' AND grade_id = '".$game_grade."' AND school_id = '".$school_id."' and user_id='".$this->session->user_id."' and gs_id = '".$skill_id."') and complexity_level=".$user_current_session." GROUP BY g.gid ORDER BY RAND() LIMIT 1");
				//echo $this->db->last_query(); exit;
			return $query->result_array();
		 }
		...
		//echo $this->db->last_query(); exit;
		 ...
		 { 
			$query = $this->db->query("INSERT INTO rand_selection SET gc_id = '".$catid."', gs_id = '".$skill_id."', gid = '".$gameid."', gp_id = '".$game_plan_id."', grade_id = '".$game_grade."', section = '".$section."', school_id = '".$school_id."',user_id='".$this->session->user_id."',created_date = '".$check_date_time."',complexity_level = '".$user_current_session."' "); 
		 } 
		 public function insertSK_RandomGames($catid,$game_plan_id,$game_grade,$uid,$skill_id,$school_id,$section,$gameid,$check_date_time)
		 {

		$query = $this->db->query("INSERT INTO sk_rand_selection SET userID='".$uid."', gc_id = '".$catid."', gs_id = '".$skill_id."', gid = ".$gameid.", gp_id = '".$game_plan_id."', grade_id = '".$game_grade."', section = '".$section."', school_id = '".$school_id."', created_date = '".date('Y-m-d h:i:s')."'");
		
			 
		 }
		public function getActualGames($game_plan_id,$game_grade,$uid,$catid,$where)
		{
			$query = $this->db->query("SELECT j.id,(select count(*) as tot_game_played from game_reports where gu_id = '".$uid."'  AND gc_id = '".$catid."' AND gs_id = e.skill_id AND gp_id = '".$game_plan_id."' AND lastupdate = '".date('Y-m-d')."') as tot_game_played ,
			(select ".$this->config->item('starslogic')."(game_score)  from game_reports where gu_id =  '".$uid."'  AND gc_id = '".$catid."' AND gs_id = e.skill_id AND gp_id = '".$game_plan_id."' AND lastupdate = '".date('Y-m-d')."') as tot_game_score ,
			(select DISTINCT puzzle_cycle from gamescore where g_id=g.gid and gu_id='".$uid."' and lastupdate='".date('Y-m-d')."' order by puzzle_cycle DESC limit 1) current_puzzle_cycle,
			(SELECT  CASE  when count(gs_id)>=10 THEN count(id) WHEN FIND_IN_SET('U',group_concat(answer_status))>=1 THEN 'TO' when count(gs_id)<10 THEN count(id) ELSE 0 END CompletedSkill FROM gamescore as gs   where gs.gs_id=e.skill_id and gu_id='".$uid."' and lastupdate='".date('Y-m-d')."' and puzzle_cycle=current_puzzle_cycle) as tot_ques_attend, 
			e.skill_id, j.name AS skill_name, g.gid, g.gname, g.img_path,g.game_html, j.icon 
			FROM users AS a
			JOIN class_plan_game AS d ON d.class_id='".$game_grade."' AND d.plan_id = '".$game_plan_id."'
			JOIN class_skill_game AS e ON e.class_id = '".$game_grade."' AND  d.game_id = e.game_id
			JOIN category_skills AS j ON e.skill_id = j.id 
			JOIN games AS g ON d.game_id = g.gid	WHERE a.id = '".$uid."' AND g.gc_id = '".$catid."' $where GROUP BY skill_id,g.gid"); 
			return $query->result_array();
		}
		/* public function getSK_ActualGames($game_plan_id,$game_grade,$uid,$catid,$where,$planwhere)
		{
			$query = $this->db->query("SELECT  g.skill_ID as skill_id,game_html,(select count(*) as tot_game_played from sk_game_reports where gu_id = '".$uid."'  AND gc_id = '".$catid."' AND g_id = g.ID AND gp_id = '".$game_plan_id."' AND lastupdate = '".date('Y-m-d')."') as tot_game_played, (select times_count from sk_games_plan where school_ID='".$this->session->school_id."'  and sk_game_ID=g.ID $planwhere) as playcount, (select name from category_skills where id=g.skill_ID) AS skill_name,(select class_id from class_plan_game where game_id=g.game_masterID $planwhere ) AS gamegrade,".$this->config->item('SK_MinPlayCount')." as SK_MinPlayCount,g.ID as gid, g.name as gname, g.image_path as img_path,(select colorcode from category_skills where id=g.skill_ID) AS colorcode,
			(select DISTINCT puzzle_cycle from sk_gamescore where g_id=g.ID and gu_id='".$uid."' and lastupdate='".date('Y-m-d')."' order by puzzle_cycle DESC limit 1) current_puzzle_cycle,
			(SELECT  CASE  when count(gs_id)>=10 THEN count(id) WHEN FIND_IN_SET('U',group_concat(answer_status))>=1 THEN 'TO' when count(gs_id)<10 THEN count(id) ELSE 0 END CompletedSkill FROM sk_gamescore as gs where gs.gs_id=g.skill_ID and gu_id='".$uid."' and lastupdate='".date('Y-m-d')."' and puzzle_cycle=current_puzzle_cycle) as tot_ques_attend   FROM sk_games AS g where 1 $where order by skill_id ");
			//echo $this->db->last_query(); exit;
			return $query->result_array();
		} */
		...
		public function getSK_ActualGames($game_plan_id,$game_grade,$uid,$catid)
		{
			...
			(select count(*) as tot_game_played from sk_game_reports where gu_id = '".$uid."'  AND gc_id = '".$catid."' AND g_id = g.ID AND gp_id = '".$game_plan_id."' AND lastupdate = '".date('Y-m-d')."') as tot_game_played,

			(select DISTINCT puzzle_cycle from sk_gamescore where g_id=g.ID and gu_id='".$uid."' and lastupdate='".date('Y-m-d')."' order by puzzle_cycle DESC limit 1) current_puzzle_cycle,

			(SELECT  CASE  when count(gs_id)>=10 THEN count(id) WHEN FIND_IN_SET('U',group_concat(answer_status))>=1 THEN 'TO' when count(gs_id)<10 THEN count(id) ELSE 0 END CompletedSkill FROM sk_gamescore as gs where gs.gs_id=g.skill_ID and gs.g_id=g.ID and gu_id='".$uid."' and lastupdate='".date('Y-m-d')."' and puzzle_cycle=current_puzzle_cycle) as tot_ques_attend
			...
			where userID='".$uid."' and date(rd.created_date)='".date('Y-m-d')."' order by cs.id asc ");
			//echo $this->db->last_query(); exit;
			return $query->result_array();
		}
		...
		$query = $this->db->query("SELECT (".$this->config->item('skilllogic')."(`game_score`)) as score ,gs_id , lastupdate FROM `game_reports` WHERE gs_id in (59,60,61,62,63) and gu_id='".$userid."' and (lastupdate between '".$this->session->astartdate."' and '".$this->session->aenddate."') and (session_id between '".$Session_StartRange."' and '".$Session_EndRange."') and session_id!=0   group by gs_id ,session_id");
	//	echo $this->db->last_query();  exit;
			return $query->result_array();
		 }
	...
	$query = $this->db->query("update users set fname = '".$sfname."',father='".$fathername."',mother='".$mothename."',gender='".$gender."',email='".$emailid."',dob='".$dob."',mobile='".$sphoneno."',address='".$address."', avatarimage=$avatarimage where id = '".$id."'");
	   ...
	   public function getgameplanid($userid)
		...
		//echo $this->db->last_query(); 
		 ...
		 {
			 //echo 'hello'; exit;
		$query = $this->db->query("insert into gamedata (gu_id,gc_id,gs_id,gp_id,g_id,total_question,attempt_question,answer,game_score,gtime,rtime,crtime,wrtime,lastupdate,created_date_time,Is_schedule) values('".$userid."','".$cid."','".$sid."','".$pid."','".$gameid."','".$total_ques."','".$attempt_ques."','".$answer."','".$score."','".$a6."','".$a7."','".$a8."','".$a9."','".$lastup_date."','".date("Y-m-d H:i:s")."','".$schedule_val."')");
		//echo $this->db->last_query(); exit;
		...
		//echo $this->db->last_query(); exit;
		 ...
		 {
			 $query = $this->db->query('SELECT ROUND(AVG(gr.game_score),2) as game_score,gs_id, lastupdate,DATE_FORMAT(`lastupdate`,"%d") as playedDate FROM game_reports gr join category_skills sk join users u WHERE gr.gu_id = u.id and u.sid = '.$school_id.' and gr.gu_id='.$uid.' and sk.id = gr.gs_id and gr.gs_id in (SELECT id FROM category_skills where category_id=1) and  lastupdate between "'.$startdate.'" and "'.$enddate.'" AND DATE_FORMAT(lastupdate, "%Y-%m")=\''.$dateQry.'\' group by lastupdate, gr.gs_id, gr.gu_id order by gr.gs_id');
		echo $this->db->last_query(); exit;
			return $query->result_array();
		...
		$query = $this->db->query("insert into sk_user_game_list(userID,planID,SessionID,weakSkills,levelid,status,created_date,session_start_range,session_end_range) values ('".$r['id']."','".$r['gp_id']."','".($maxsession)."','".implode (",", $month_array_skill)."','".implode (",", $levelid)."',0,'".date("Y-m-d")."','".$Session_StartRange."','".$Session_EndRange."')");
		//echo $this->db->last_query(); exit;
...
from(select gamescore,gs_id,lastupdate,playedMonth,gu_id from (SELECT (AVG(game_score)) as gamescore,gs_id,lastupdate,DATE_FORMAT(lastupdate,'%m') as playedMonth,gu_id  FROM game_reports join users as u on u.id=gu_id WHERE gs_id in(63) and  lastupdate between '".$this->session->astartdate."' and '".$this->session->aenddate."' and sid='".$this->session->school_id."' and grade_id='".$this->session->game_grade."' group by gu_id,gs_id,lastupdate) as a1 where gamescore!=0 group by  gs_id,lastupdate order by gamescore ASC)a5 inner join range_values r on a5.gamescore >startrange  and a5.gamescore <=endrange group by r.startRange, r.endRange order by startRange asc )a2 group by totalCount order by totalCount desc limit 1");
	...
	{
		$query = $this->db->query("select count(ID) as isenable from sk_user_game_list where userID=".$userid." and planID=".$plan_id." and status=0  and (".$this->session->Next_Session_id."  between session_start_range and session_end_range) ");
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query('INSERT INTO user_login_log(userid,sessionid,first_login_datetime,created_date,lastupdate,logout_date,ip,country,region,city,browser,isp,status)VALUES("'.$userid.'","'.$sessionid.'","'.date("Y-m-d H:i:s").'","'.date("Y-m-d H:i:s").'","'.date("Y-m-d H:i:s").'","'.date("Y-m-d H:i:s").'", "'.$ip.'","'.$country.'","'.$region.'","'.$city.'","'.$browser.'","'.$isp.'","'.$status.'")');
		return $query;
	}
 public function update_login_log($userid,$sessionid,$now)
		...
		{
			$query = $this->db->query("select count(distinct(lastupdate)) as playedDate from game_reports gr where gr.gu_id='".$userid."' and lastupdate between '".$this->session->astartdate."' and '".$this->session->aenddate."' and session_id!=0 and lastupdate!='".date("Y-m-d")."' ");
			return $query->result_array();
		}
...
//echo $this->db->last_query(); exit;
	...
	public function getAsapBspi($username)
	{
		...
		{
			//$this->load->library('Asapo');
			$query = $this->db->query("SELECT  sum(game_score)/5 as bspi FROM html5schools_assessmentso_1920.game_reports join html5schools_assessmentso_1920.users as u on u.id=gu_id where u.username='".$username."' and u.status=1");
			//echo $this->multipledb->db->last_query(); exit;
		}
		else
		{
			//$this->load->library('Multipledb');
			$query = $this->db->query("SELECT  sum(game_score)/5 as bspi FROM html5schools_assessments_1920.game_reports join html5schools_assessments_1920.users as u on u.id=gu_id where u.username='".$username."' and u.status=1");
		}
		return $query->result_array();
	}
	...
	public function getAsapcore($username)
	{
		...
		if($this->session->school_id==130 || $this->session->school_id==131 || $this->session->school_id==132 || $this->session->school_id==133 || $this->session->school_id==136 || $this->session->school_id==138 || $this->session->school_id==139 || $this->session->school_id==2)
		{
			...
			(select game_score from  html5schools_assessmentso_1920.game_reports where gs_id=59  and gu_id=a1.id ) as ME,
			(select game_score from  html5schools_assessmentso_1920.game_reports where gs_id=60  and gu_id=a1.id ) as VP,
			(select game_score from  html5schools_assessmentso_1920.game_reports where gs_id=61  and gu_id=a1.id ) as FA,
			(select game_score from  html5schools_assessmentso_1920.game_reports where gs_id=62  and gu_id=a1.id ) as PS,			
			(select game_score from  html5schools_assessmentso_1920.game_reports where gs_id=63  and gu_id=a1.id ) as LI			
			from (select id from html5schools_assessmentso_1920.users where username='".$username."' and status=1) a1 ");
			//echo $this->db->last_query(); exit;
		}
		else
		{
			//$this->load->library('Multipledb');
			$query = $this->db->query("select 
			(select game_score from  html5schools_assessments_1920.game_reports where gs_id=59  and gu_id=a1.id ) as ME,
			(select game_score from  html5schools_assessments_1920.game_reports where gs_id=60  and gu_id=a1.id ) as VP,
			(select game_score from  html5schools_assessments_1920.game_reports where gs_id=61  and gu_id=a1.id ) as FA,
			(select game_score from  html5schools_assessments_1920.game_reports where gs_id=62  and gu_id=a1.id ) as PS,			
			(select game_score from  html5schools_assessments_1920.game_reports where gs_id=63  and gu_id=a1.id ) as LI			
			from (select id from html5schools_assessments_1920.users where username='".$username."' and status=1) a1 ");
		}
			return $query->result_array();
	}
	...
	{
		//$this->load->library('Multipledb');
		$query = $this->db->query("select id,portal_type,sid,IS_ASAP,gp_id from html5schools_assessments_1920.users where username='".$username."' and password=SHA1(CONCAT(salt1,'".$password."',salt2)) and status=1");
		//echo $this->multipledb->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{
		 //$this->load->library('Asapo');
		$query = $this->db->query("select id,portal_type,sid,IS_ASAP,gp_id from html5schools_assessmentso_1920.users where username='".$username."' and password=SHA1(CONCAT(salt1,'".$password."',salt2)) and status=1");
 ...
 from users where  id='".$userid."' ");
	...
	public function myTrophiesAll_opt($userid,$startdate,$enddate,$Session_StartRange,$Session_EndRange)
	{
		...
		gr.lastupdate AS lastupdate from game_reports gr where gs_id IN(59,60,61,62,63) and gu_id='".$userid."' and (session_id <='".$Session_EndRange."') and session_id!=0 group by session_id,gs_id,gr.gu_id) as a1 group by session_id,id");
		//echo $this->db->last_query(); exit;
		return $query->result_array();
 ...
 from users where  id='".$userid."' ");
	...
	public function getMyCurrentTrophies_opt($userid,$start_range,$end_range)
	{
		$query = $this->db->query("select c.id as catid,c.name,sum(diamond) as diamond,sum(gold) as gold,sum(silver) as silver from 
			...
			where gs_id IN(59,60,61,62,63) and (session_id between '".$start_range."' and '".$end_range."') and session_id!=0 and  gu_id='".$userid."'  group by gr.session_id,gs_id) as a1 group by id) a2 right join category_skills as c on c.id=catid  where category_id=1 group by c.id ");
...
//	echo $this->db->last_query(); exit;
	...
	{
		$this->load->library('School1819');
			$query = $this->school1819->db->query('select id,sid FROM users a WHERE username="'.$username.'" AND password=SHA1(CONCAT(salt1,"'.$pwd.'",salt2)) AND status=1');
	...
	{
		$query = $this->db->query("select skill_id,median_value from skillkit_master where grade_id=".$grade_id." order by skill_id ASC");
		return $query->result_array();
	}
	public function getDefaultCycleData($Session_StartRange,$Session_EndRange,$session_curid)
	{
		$query = $this->db->query("select * from cycle_master where status=1 and range_start <=".$session_curid." ");
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	public function UpdateTodaySession($userid,$todaydate)
	{
		$query = $this->db->query('CALL UpdateTodaySessionnew("'.$userid.'","'.$todaydate.'")');
		mysqli_next_result($this->db->conn_id);
		return $query->result_array();
	}
	...
	public function getCurrentBSPIName($Session_StartRange,$Session_EndRange,$session_curid)
	{
		...
		$query = $this->db->query("select * from cycle_master where status=1 and ".$session_curid." between range_start and range_end ");
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query('select (select count(*) from sk_gamedata where gu_id='.$userid.' and lastupdate="'.$todaydate.'") as playedcount,(select count(*) from sk_rand_selection where userID='.$userid.' and date(created_date)="'.$todaydate.'") as assignedcount');
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query('CALL UpdateSkillkitSession("'.$userid.'","'.$todaydate.'","'.$start_range.'","'.$end_range.'")');
		mysqli_next_result($this->db->conn_id);
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query("select count(DISTINCT gs_id) as playedskillcount from gamedata where gu_id=".$userid." and lastupdate='".$todaydate."' ");
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	public function Update_Timer_StartTime($userid,$sessionid,$now)
	{
		$query = $this->db->query('update user_login_log set created_date="'.$now.'",lastupdate="'.$now.'",logout_date="'.$now.'" where userid="'.$userid.'" and sessionid="'.$sessionid.'"');
		return $query;
	}
	/*NEW FEATURES*/
	...
	public function getPuzzle_cycle($gid,$uid)
	{
		...
		join users as u on u.id=gu_id where g_id='".$gid."' and u.id='".$uid."' and lastupdate='".date("Y-m-d")."' and status=1 group by puzzle_cycle
	...
	public function InsertSingleGameScore($uid,$SID,$GID,$ResponseTime,$BalaceTime,$CorrectAnswer,$UserAnswer,$AnswerStaus,$QNO,$SCORE,$TimeOverStatus,$puzzle_cycle,$curdate,$curdatetime)
	{
		...
		$query = $this->db->query("INSERT INTO gamescore(gu_id, gs_id, g_id, que_id, answer, useranswer, game_score, answer_status, timeoverstatus, responsetime, balancetime, lastupdate, creation_date, modified_date, puzzle_cycle)VALUES('".$uid."','".$SID."','".$GID."','".$QNO."','".$CorrectAnswer."','".$UserAnswer."','".$SCORE."','".$AnswerStaus."','".$TimeOverStatus."','".$ResponseTime."','".$BalaceTime."','".$curdate."','".$curdatetime."','".$curdatetime."','".$puzzle_cycle."') "); 
	}
	...
	{
		$query = $this->db->query('select * from gamedata where gu_id="'.$userid.'" and lastupdate="'.$todaydate.'" and g_id='.$gid.' and puzzle_cycle='.$current_Cycle.' ');
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query(" SELECT count(*) as qcnts,sum(game_score) as scores,MIN(balancetime) as timerval,GROUP_CONCAT(que_id) as qvalues,sum(responsetime) as rsptime,max(answer) as crtcnt,GROUP_CONCAT(game_score) as questionscore,GROUP_CONCAT(useranswer) as useranswer FROM sk_gamescore  WHERE g_id='".$gid."' and gu_id='".$uid."' and puzzle_cycle='".$puzzlecycle."' and lastupdate='".date("Y-m-d")."' ");
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	public function getSKPlayedPuzzleCount($userid,$todaydate,$gid,$current_Cycle)
	{
		$query = $this->db->query('select count(gu_id) as playedcount from sk_gamescore where gu_id = "'.$userid.'" and lastupdate ="'.$todaydate.'" and g_id='.$gid.' and puzzle_cycle='.$current_Cycle.' ');
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{	
		$query = $this->db->query('CALL SK_GameDataInsert("'.$uid.'","'.$SID.'","'.$GID.'","'.$ResponseTime.'","'.$BalaceTime.'","'.$CorrectAnswer.'","'.$UserAnswer.'","'.$AnswerStaus.'","'.$QNO.'","'.$SCORE.'","'.$TimeOverStatus.'","'.$puzzle_cycle.'","'.$curdate.'","'.$curdatetime.'","'.$gametime.'","'.$gp_id.'")'); 
	...
	{
		$query = $this->db->query('select created_date,group_concat(gid) as gid from(select date(created_date) as created_date,gid as gid from rand_selection where user_id='.$userid.' order by date(created_date) DESC limit 5) as a1');
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	public function checkPuzzlesAllPlayed($userid,$assigneddate,$gid)
	{
		$query = $this->db->query('select count(DISTINCT gs_id) as playedcount from gamedata where gu_id='.$userid.' and lastupdate="'.$assigneddate.'"  and g_id IN('.$gid.') ');
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{ 
		...
		$query = $this->db->query("INSERT INTO rand_selection(gc_id, gs_id, gid, gp_id, grade_id, section, school_id, user_id, created_date) select ".$catid." as gc_id,gs_id,gid,".$game_plan_id." as gp_id,".$game_grade." as grade_id,'".$section."' as section,'".$school_id."' as school_id,".$userid.",'".$check_date_time."' from games where gid in(".$gid.") "); 
	}
	public function UpdateUserCurrentSessionLevel($userid,$SESSIONLEVEL)
	{
		$query = $this->db->query('Update users set current_session='.$SESSIONLEVEL.' where id='.$userid.' '); 
	}
	public function getSK_AssignedSkills($uid)
	{
		$query = $this->db->query('select weakSkills,levelid  from sk_user_game_list where userID='.$uid.' and status=0'); 
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query('CALL sp_userlogin("'.$userid.'","'.$uniqueId.'","'.$grade_id.'","'.$sid.'","'.$section.'","'.$ip.'","'.$txcountry.'","'.$txregion.'","'.$txcity.'","'.$txisp.'","'.$browser.'","'.$status.'","'.$todaydate.'","'.$now.'")');
		mysqli_next_result($this->db->conn_id);
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query('CALL SkillkitGameAssign("'.$user_id.'","'.$game_grade.'","'.$confignoofdaysplay.'","'.$Session_StartRange.'","'.$Session_EndRange.'","'.$plan_id.'","'.$Next_Session_StartRange.'","'.$Next_Session_EndRange.'","'.$CurDate.'","'.$astartdate.'","'.$aenddate.'","'.$CurDateTime.'","'.$IntervalRange.'")');
		mysqli_next_result($this->db->conn_id);
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
	...
	{	 
		$query = $this->db->query('CALL TodayTimerInsert("'.$user_id.'","'.$curdate.'","'.$curdatetime.'")');
		mysqli_next_result($this->db->conn_id);
		//echo $this->db->last_query(); exit;
		return $query->result_array();
	}
