...
class Home extends CI_Controller {
	...
	{
		// Call the CI_Model constructor session_status() === PHP_SESSION_NONE 
			parent::__construct();
		...
		/*------------before login language change ----------------*/	 	
		if($this->session->user_id=="" || !isset($this->session->user_id))
		{  
			$languageid = $this->uri->segment(4);//echo $languageid;exit;		
			$ci =& get_instance();
	...
	{ 
		...
		{ 
			...
			{ 
				...
				$bstartdate  = strtotime($data['braintest'][0]['startdate']);
				$btenddate  = strtotime($data['braintest'][0]['enddate']);
				$curdate = date('Y-m-d H:i:s');
				$curdatetime = strtotime($curdate);
				if($bstartdate<=$curdatetime && $btenddate >= $curdatetime && $data['query'][0]->is_playedhots==0)
				{
					$isbraintest=1; //Enabled
					$this->session->set_userdata('isbraintest',$isbraintest);
					$this->session->set_userdata('is_playedhots',0);
					$this->session->set_userdata('bt_menuenable',1);
				}
				else if($data['query'][0]->is_playedhots==1)
				{
					$isbraintest=1; //Enabled
					$this->session->set_userdata('bt_menuenable',1);
				}
				...
				{
					$this->session->set_userdata('bt_menuenable',0);
				}
				/* ------------ Hots olympiad end -----------------------*/
				...
				{
				$Sclass = 'commonmsg';
				$greetings_content ='<div style="padding:7%;"><h3 style="color:#ff6600;">'.$this->lang->line("in_hey").' '.$fname.'..</h3><p style="font-size: 23px; color:#000;">'.$this->lang->line("in_welcometohots").'</p></div>';
				} 
				else if((isset($login_count) && $login_count<=1) || $prelogin_date == "0000-00-00")
				{
					$Fclass = 'firstlogincls';
					$greetings_content = $this->lang->line("in_session_rules"); 

				}
				elseif((isset($previou_sessiondate) && $previou_sessiondate==0))
				{
					$Sclass = 'commonmsg';
				$greetings_content ='<div style="padding:7%;"><h3 style="color:#ff6600;">'.$this->lang->line("in_hey").' '.$fname.'..</h3><p style="font-size: 23px; color:#000;">'.$this->lang->line("in_welcomebckmsg").' </p></div>';
				}
				...
				{
					$Sclass = 'commonmsg';
				$greetings_content ='<div style="padding:8%;"><p style="font-size: 23px; color:#000;">'.$this->lang->line("in_welcomebckmsg2").'</p><p style="font-size: 23px; color:#000;">'.$this->lang->line("in_funmsg2").'</p></div>';
				}
				...
				if($data['query'][0]->language_id==101)
				{
					$gamelanguage='EnglishQuestionText';
					$gamepath ='clp';
				}
				else if($data['query'][0]->language_id==104)
				{
					$gamelanguage='GujaratiQuestionText';
					$gamepath ='clpx';
				}
				else
				{
				...
				$this->session->set_userdata(array(
					...
					'gamesound'=>$data['query'][0]->user_game_sound,
					'gamelanguage'=>$gamelanguage,
					'language_id'=>$data['query'][0]->language_id
				));
			...
			$level_calc=($data['query'][0]->current_session%$confignoofdaysplay);
			if($level_calc==0)
			{
				$this->session->set_userdata('user_current_session',$confignoofdaysplay);
			}
			else
			{
			...
			//	Hots Olympiad
			if($this->session->isbraintest=='1' && $this->session->is_playedhots==0)
			{
				$this->session->set_userdata('TimerRunningStatus','N'); // Don't run the Timer Now.
			}
			...
			if($this->session->isbraintest==1)
			{
				echo $result = 'BT';
			}
			elseif($data['skillkitenable'][0]['isenable']>=1)
			{
				echo $result=0; exit;
...
$arrSkills=$this->Assessment_model->getSkillsRandom($catid);
	...
	public function gamesajax()
	{
		...
		{	 
			...
			//$gameid = $data['gameid'][0]['gid'];
			$this->session->set_userdata(array('currentgameid'=> $_POST['gameid']));
			$this->session->set_userdata(array( 'isskillkit'=> $_POST['skillkit'] ));
			echo $this->session->currentgameid;exit;
		}
		else
	...
	public function profile()
	{
		...
		$data['getgamesound'] = $this->Assessment_model->get_gamesound();
		$data['getlanguage'] = $this->Assessment_model->get_language();
		//echo "<pre>";print_r ($data['getlanguage']);exit;
...
{   
	...
	{	if($this->session->avatarimage!=''){$src=base_url()."".$this->session->avatarimage;}else{$src=base_url()."assets/images/".'avatar.png';}
		...
		<tbody><tr> 
		<td><img src="'.$src.'" width="40" height="40" class="img-circle"></td>
		<td>'.$this->lang->line("in_welcome").' '.$this->session->fname.','.$this->lang->line("in_welcome_greatdaytxt").' </td>
		</tr>
		</tbody></table>
		</li>';
	}
...
$res_tot_memory_i=$res_tot_vp_i=$res_tot_fa_i= $res_tot_ps_i=$res_tot_lang_i=0;
	...
	public function getOverAllTopperList()
	{
		...
		{	
			...
			{	
				$data1.="<li class='news-item'><div class='norecord'>".$this->lang->line("in_norecord")."</div></li>";
			}
...
public function isUser()
{
	...
	*/
	...
	if (!empty($_POST))
	{ 
		$data['query']= $this->Assessment_model->checkUser($this->input->post('email'),$this->input->post('pwd'));
		//$arrisUser= $this->Assessment_model->CheckisUser('sundar','skillangels');
	
	/*................. Dubai School features ...............*/
		//$data['new_features'] = $this->Assessment_model->new_features($data['query'][0]->sid);
		//$new_features =$data['new_features'][0]->new_features;

		//if($new_features=="Y") // N - new feature, O - old features
		if(1!=1)
		{ 	 
			/*.............Restrict_time ...............*/
			if(isset($data['query'][0]->restrict_time))
			{				
				$restime =$data['query'][0]->restrict_time;
				$rt=array();
				$arr_rt = explode(",",$restime);
				foreach($arr_rt as $res)				
				{ 
					$arr_rt1 = explode("-",$res);
					$starttime = $arr_rt1[0];
					$stime = date("His", strtotime($starttime));				
					$endtime = $arr_rt1[1];
					$etime = date("His", strtotime($endtime));				
					$currentTime = time();
					if ((date('His',$currentTime) > $stime) && (date('His',$currentTime) < $etime ))
					{
						array_push($rt,"TRUE");						
					}
					else						
			...
			/*.............holidays ...............*/ 				
				if(isset($data['query'][0]->holidays))
				{
					$hdays =$data['query'][0]->holidays;
					if($hdays=="DA")
					{
						$data['holidays'] = $this->Assessment_model->scl_holidays($data['query'][0]->sid);
						$leave_date = $data['holidays'][0]['todayleave'];					
...
public function getnextsession()
{
	...
	$response=array(
		...
		'dateday'=>$this->lang->line("in_nextchallenge").''.date("d-m-Y", strtotime($data['getnxtsession'][0]['selected_date'])).' at '.date("g:i A", strtotime($data['getnxtsession'][0]['starttime'])),
	...
	public function getAsapBspi()
	{
		...
		$this->session->set_userdata('asap_bspi',$arrbspi[0]['bspi']);
		$this->session->set_userdata('clp_current_status', $CurrentBSPIName[0]['name']);
		//return $response;
	}
...
public function getSkillChart()
{
	...
	else
	{
		...
		$data['bspi']=$tsivalue[0]['tsi'];
		//$data['skills'] = $this->Assessment_model->getskills();
		$data['skills'] = $this->Assessment_model->getWeakSkills($WeakSkills);
	}
...
public function UpdateSkillkitSession()
{
	...
	$TodayDate=date('Y-m-d');
	$SessionLevel=$this->Assessment_model->UpdateSkillkitSession($this->session->user_id,$TodayDate,$this->session->Next_Session_StartRange,$this->session->Next_Session_EndRange);
	//$cyclevalue=$this->Assessment_model->getcyclevalue();
...
public function getASAPScoreData($type=null)
{
	...
	$response=array(
		...
		'PuzzlesAttempted'=>'',
		'Crownies'=>'',
		'BSPI'=>round($this->session->asap_bspi, 2),
		'MEMORY'=>round($arrvalskillscore[0]['ME'], 2),
		'VISUAL'=>round($arrvalskillscore[0]['VP'], 2),
		'FOCUS'=>round($arrvalskillscore[0]['FA'], 2),
		'PROBLEM'=>round($arrvalskillscore[0]['PS'], 2),
...
public function feedback()
	...
	public function StaffFeedbackInsert()
	{
		...
		if($qus1!='' && $qus2!='' && $qus3!='' && $qus4!='' && $qus5!='' && $staffid!='')
		{
			...
			$arrResult=array("response"=>"1","msg"=>$this->lang->line("in_thankforfeedback"));
			echo json_encode($arrResult);exit;
		}
		else
		{
			$arrResult=array("response"=>"0","msg"=>$this->lang->line("in_providevalues"));
			echo json_encode($arrResult);exit;
		}
	}
/* ------------------------ Staff FeedBack ------------------------ */
...
public function iaschallenge()
{
	...
	$curdate = date('Y-m-d H:i:s');
	$currdatetime = strtotime($curdate); ;
	if($this->session->bstartdate<=$currdatetime ){}
	else{redirect('index.php/home/dashboard');}
	$data['getlanguage'] = $this->Assessment_model->get_language();
 	$data['GamePlayedDataA']=$this->Assessment_model->GamePlayedDataA($userid,1);
 	$data['GamePlayedDataB']=$this->Assessment_model->GamePlayedDataB($userid,2);
	$data['get_btscore']=$this->Assessment_model->get_btscore($userid);
	$data['skillkitenable']=$this->Assessment_model->IsSkillkitExist($this->session->user_id,$this->session->game_plan_id);
	//echo "<pre>";print_r($data['skillkitenable']);exit;
	if($this->session->is_playedhots==1)
	{  
		//echo $this->session->is_playedhots."==".$this->session->skillkitactive;exit;
...
{	
	...
	$curdatetime = strtotime($currdate);
	if(($this->session->bstartdate <= $curdatetime && $this->session->benddate >= $curdatetime) && $this->session->isbraintest==1) 
	{	
		$response=1;
	}
	else
...
public function get_info_hots()
{
	$gname=$this->input->post('gname');
	$uid=$this->session->user_id; 
	$gid=$this->session->currentgameid; 
	//$uid=38332; 
	$curdate=date('Y-m-d'); 
	...
	if($gname!='' && $uid!='')
	{
		...
		if(isset($gid)!='' && $gid!='')
		{
			//$arrofpuzzle=$this->Assessment_model->getPuzzle_cycle($gid,$uid);
			$current_Cycle=1;
			
			$arrPlyedDetails=$this->Assessment_model->getGamePlayedDetails_hots($gid,$uid,$current_Cycle,$curdate);
			//echo "<pre/>";print_r($arrPlyedDetails);exit;
			$arr=array(
				"gname"=>$gname,
				"gameid"=>$gid,
...
public function scoreupdate_hots()
{
	...
	if(isset($_REQUEST["SID"]) && isset($_REQUEST["GID"]) && isset($_REQUEST["RT"]) && isset($_REQUEST["TV"]) && isset($_REQUEST["CA"]) && isset($_REQUEST["UA"]) && isset($_REQUEST['AS']) && isset($_REQUEST['QNO']) && isset($_REQUEST['SCORE']) && isset($_REQUEST['puzzle_cycle']) && isset($_REQUEST['TOS']))
	{
		...
		$currentlangid=$this->session->currentlanguage; 
		//$uid=38332; 
		$SID=$_REQUEST["SID"]; 
		$GID=$_REQUEST["GID"];
		if($GID==2)
		...
		$todaydaste = date('Y-m-d'); 
		$arrofPlayedCount=$this->Assessment_model->getPlayedPuzzleCount_hots($uid,$todaydaste,$GID);
	
		if($arrofPlayedCount[0]['playedcount']>($currentquestion-1))
		{
			echo -2;exit;
		}
		...
		$arrofpuzzle=$this->Assessment_model->InsertGameData_hots($uid,$SID,$GID,$ResponseTime,$BalaceTime,$CorrectAnswer,$UserAnswer,$AnswerStaus,$QNO,$SCORE,$TimeOverStatus,$puzzle_cycle,$curdate,$curdatetime,$gametime,$this->session->gp_id,$currentquestion,$btlevel,$currentlangid);
		
		$arrofresult=$this->Assessment_model->checkandUpgradeUserStatus_hots($uid,$this->session->username);
		$this->session->set_userdata('is_playedhots',$arrofresult[0]['result']);
	...
	public function updategamesound()
	{	
		//echo "hlo";exit;
		if($this->session->user_id=="" || !isset($this->session->user_id)){redirect('index.php');}
		$userid = $this->session->id;//echo $userid;exit;
		$gamesound = $this->input->post('gamesound');//echo $gamesound;exit;		
		$this->session->set_userdata(array(							 
			'gamesound'  => $gamesound
		));		
		
		//echo $_SESSION['gamesound'];exit;
		$data['updategamesound'] = $this->Assessment_model->updategamesound($gamesound,$userid);
		$this->load->view('myprofile/newmyprofile', $data);
	}

	/*-------------update language which is get from the ajax --*/
